...
def download_and_prefetch_dataset():
  (train_cifar, test_cifar), ds_info = tfds.load(name="cifar10", split=['train', 'test'], as_supervised=True, with_info=True)

  # Applying normalization before `ds.cache()` to re-use it.
  # Note: Random transformations (e.g. images augmentations) should be applied
  # after both `ds.cache()` (to avoid caching randomness) and `ds.batch()` (for
  # vectorization [1]).
  train_cifar = train_cifar.map(normalize_img, num_parallel_calls=tf.data.experimental.AUTOTUNE)
  train_cifar = train_cifar.cache()
  test_cifar = test_cifar.map(normalize_img, num_parallel_calls=tf.data.experimental.AUTOTUNE)
  test_cifar = test_cifar.cache()

  # For true randomness, we set the shuffle buffer to the full dataset size.
  print("There are " + str(ds_info.splits['train'].num_examples) + " training samples")
  train_cifar = train_cifar.shuffle(ds_info.splits['train'].num_examples)
  test_cifar = test_cifar.shuffle(ds_info.splits['test'].num_examples)

  # Batch after shuffling to get unique batches at each epoch.
  train_cifar = train_cifar.batch(NB_SAMPLES)
  train_cifar = train_cifar.prefetch(tf.data.experimental.AUTOTUNE)
  test_cifar = test_cifar.batch(NB_SAMPLES)
  test_cifar = test_cifar.prefetch(tf.data.experimental.AUTOTUNE)
...
def train_model(model, batch_size, dataset_flag):
  # Load the dataset
  if dataset_flag == 0:
    ...
    load_type = "prefetch"
    train_batch,test_batch = download_and_prefetch_dataset()
...
def main(argv):
  ...
  train_model(model, batch_size, flags.prefetch_dataset)
